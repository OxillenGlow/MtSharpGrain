...
Register **repeating** or **one-time** callbacks that run on the mod’s **virtual thread** (not the render thread).
Unlike `Engine.onTick`, these are **not tied to the game loop** and can fire at **custom intervals**.

```js
// Engine.setInterval(fn, ms) → timerId
// Runs `fn` every `ms` milliseconds. Returns a timer ID for cancellation.
const timerId = Engine.setInterval(function() {
    console.log("Runs every 500ms");
}, 500);

// Engine.clearInterval(timerId)
// Cancels a repeating timer.
Engine.clearInterval(timerId);

// Engine.setTimeout(fn, ms) → timerId
...
Threading: Callbacks run on the mod’s virtual thread (not the render thread).

No arguments: fn receives no parameters (unlike Engine.onTick’s tpf and tag).

Timer IDs: Unique per mod pack. Automatically cleaned up when the mod unloads.

Precision: Uses Java’s ScheduledExecutorService for accurate timing.
...
- `Engine.onTick` callbacks run on the **render thread**. Timer callbacks (`setInterval`/`setTimeout`) run on the **mod’s virtual thread**. Do not spin-wait or do heavy computation inside callbacks — keep them short or else they will be useless and time out.
...
Engine:
  Engine.onTick(fn, tag)   // fn(tpf, tag); only tag "Update" runs automatically every ~0.2 seconds;
                           // other tags only fire when a Gui element with the same tag (same pack) is
                           // clicked. 5 consecutive throws auto-disables
                           // the callback.
  Engine.onBlockChange(fn) // fn(x,y,z,blockId) -> bool; false or throw rejects the edit; runs for ALL packs' edits

  // IN MTAHARPGRAIN MODS, TO CALLBACK SOME TIME LATER OR AUTO UPDATE, USE THE FOLLOWING:

  Engine.setInterval(fn, ms)    // -> timerId; repeat fn every ms milliseconds
  Engine.clearInterval(id)       // cancel a repeating timer
  Engine.setTimeout(fn, ms)     // -> timerId; run fn once after ms milliseconds
  Engine.clearTimeout(id)        // cancel a pending timeout
...
Packs whose **folder name ends with `MAIN`** (case-sensitive), e.g.
`hudMAIN`, `trackerMAIN`, run on the **render / main thread** instead of a
virtual thread. Use two mod packs with Mod.send and receive api for large projects.
...
For actual source code, go to [folder here](https://github.com/OxillenGlow/MtSharpGrain/tree/main/src/main/java/com/mtsharpgrain/js/mainthread)
